...
from tornado_rest_client.version import __version__

PACKAGE = "tornado_rest_client"
...
class Pep8Command(Command):
    ...
    def run(self):
        ...
        if report.total_errors:
            sys.exit("ERROR: Pep8 failed with exit %d errors" % report.total_errors)
...
class UnitTestCommand(Command):
    ...
    user_options = []
    args = [PACKAGE, "--with-coverage", "--cover-package=%s" % PACKAGE, "-v"]
